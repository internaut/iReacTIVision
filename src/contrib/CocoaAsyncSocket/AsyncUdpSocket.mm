...
- (BOOL)joinMulticastGroup:(NSString *)group withAddress:(NSString *)address error:(NSError **)errPtr
{
	...
	{
		const struct sockaddr_in* nativeAddress = (const struct sockaddr_in*)[address4 bytes];
		const struct sockaddr_in* nativeGroup = (const struct sockaddr_in*)[group4 bytes];
	...
	{
		const struct sockaddr_in6* nativeGroup = (const struct sockaddr_in6*)[group6 bytes];
...
- (void)doSend:(CFSocketRef)theSocket
{
	if(theCurrentSend != nil)
	{
		...
		if([self canAcceptBytes:theSocket])
		{
			...
			{
				const struct sockaddr *dst  = (const struct sockaddr *)[theCurrentSend->address bytes];
				NSUInteger dstSize = [theCurrentSend->address length];
				
				result = sendto((int)theNativeSocket, buf, (size_t)bufSize, 0, dst, (socklen_t)dstSize);
			}
